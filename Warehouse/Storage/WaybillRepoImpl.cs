...
namespace Warehouse.Storage
{
    internal class WaybillRepoImpl : CrudRepo<WaybillEntity>
    {
        ...
        {
            string query = $"SELECT waybill_id, contractor.title, employee.surname, finished_product.title, document_number, car_owner, date, vehicle, shipper, consignor, loading_point, unloading_point, vehicle_number, guide_list_number, route_number, driver, trans_type FROM waybill, employee, contractor, finished_product WHERE waybill.employee_id = employee.employee_id AND waybill.contractor_id = contractor.contractor_id AND finished_product.product_id = waybill.product_id";
