...
namespace Warehouse.Service
{
    internal class DeliveryNoteOutput
    {
        ...
        {
            string deliveryNoteQuery = $"SELECT delivery_note.delivery_note_id, division.division_type, date, assignment FROM delivery_note, division WHERE delivery_note.division_id = division.division_id";

            string deliveryNoteCompositionQuery = $"SELECT finished_product.title, delivery_note_composition.unit, requested, released, price, amount FROM delivery_note_composition, delivery_note, finished_product WHERE delivery_note_composition.delivery_note_id = delivery_note.delivery_note_id AND delivery_note_composition.product_id = finished_product.product_id AND delivery_note_composition.delivery_note_id = '{deliveryId}'";

            string fileName = "Требования накладная";

            database.checkConnection();

            using (SqlCommand sqlCommand = new SqlCommand(deliveryNoteQuery, database.getSqlConnection()))
            {
            ...
            using (SqlCommand sqlCommand = new SqlCommand(deliveryNoteCompositionQuery, database.getSqlConnection()))
            {
                SqlDataReader reader = sqlCommand.ExecuteReader();
